...
export default class ShowMovie extends Component {
  ...
  async componentDidMount() {
    ...
    if (token !== null) {
      ...
      const [movie, favMovieArray] = await Promise.all([
        axios.get(
          `https://api.themoviedb.org/3/movie/${this.props.match.params.id}?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US`
        ),
  ...
  addHandler = async (e) => {
    const favMovieArray = await axios.post(
      `/user/${this.state.uid}/favourite/${this.props.match.params.id}`
    );
    this.setState({
      favMovieArray: favMovieArray.data,
      isrDisabled: false,
  ...
  removeHandler = async (e) => {
    const favMovieArray = await axios.delete(
      `/user/${this.state.uid}/favourite/${this.props.match.params.id}`
    );
    this.setState({
      favMovieArray: favMovieArray.data,
            ...
            <Card.Body>
              ...
              <Card.Text>
                ...
                <h6 className="display-6">
                  Rating : {this.state.vote_average}
                </h6>
              </Card.Text>
              <Card.Text>{this.state.overview}</Card.Text>
              {!this.state.favMovieArray.includes(
                this.props.match.params.id
              ) ? (
                <Button
                  onClick={this.aDisableHandler}
                  variant="secondary"
                  disabled={this.state.isaDisabled}
                >
                  Add to Favourites
                </Button>
              ) : (
                <Button
                  onClick={this.rDisableHandler}
                  variant="danger"
