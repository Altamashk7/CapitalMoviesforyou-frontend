...
export default class PopMovieList extends Component {
  ...
  async componentDidMount() {
    const fetchmovies = await axios.get(
      `https://api.themoviedb.org/3/movie/popular?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&page=${this.state.page}`
    );
  ...
  pPageHandler = async () => {
    ...
    const fetchmovies = await axios.get(
      `https://api.themoviedb.org/3/movie/popular?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&page=${newPage}`
    );
    this.setState({ movies: fetchmovies.data.results });
