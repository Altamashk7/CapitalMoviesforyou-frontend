...
export default class LatMovieList extends Component {
  ...
  async componentDidMount() {
    const fetchmovies = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US&page=${this.state.page}`
    );
  ...
  pPageHandler = async () => {
    ...
    const fetchmovies = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US&page=${newPage}`
    );
    this.setState({ movies: fetchmovies.data.results });
